...
class Connector:
    ...
    def __init__(self):
        ...
        self.couchdbserver0 = data.Server("http://%s:%s@%s:5984/" % (self.user, self.pw, self.ip0))
        self.couchdbserver2 = data.Server("http://%s:%s@%s:5984/" % (self.user, self.pw, self.ip2))
        self.couchdbserver3 = data.Server("http://%s:%s@%s:5984/" % (self.user, self.pw, self.ip3))
        ...
        self.TWITTERDB_DEMO_NAME = 'twitter_demo_db'
        self.TWITTERDB_DEMO_RESULTS_NAME = 'twitter_demo_results_db'
        ...
        self.twitterdb_demo_pub = self.create_select_db(self.TWITTERDB_DEMO_NAME, self.couchdbserver_pub)
        self.twitterdb_demo_results_pub = self.create_select_db(self.TWITTERDB_DEMO_RESULTS_NAME,
                                                                self.couchdbserver_pub)
        self.twitterdb_demo_results_pub = self.create_select_db(self.TWITTERDB_DEMO_RESULTS_NAME,
                                                                self.couchdbserver_pub)
        self.twitterdb_demo_results_pub = self.create_select_db(self.TWITTERDB_DEMO_RESULTS_NAME,
                                                                self.couchdbserver_pub)
        ...
        self.twitterdb_pub_result = self.create_select_db(self.TWITTERDB_PRO_NAME_RESULT, self.couchdbserver_pub)
